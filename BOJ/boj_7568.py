...
# N명의 집단에서 각 사람의 덩치 등수는 
# 자신보다 더 "큰 덩치"의 사람의 수로 정해진다. 
...
N = int(input())
people = [] # 키와 무게를 담는 리스트
for t in range(N): # N 번 반복
    people.append(list(map(int, input().split()))) # 리스트안의 리스트
for i in people: # people 리스트 안의 키와 몸무게에서
    ranking = 1 # ranking 이라는 변수를 일찌감치 선언.
    for j in people: 
