...
for i in mov:
    print(i.director_name,i.genre,i.title,i.opening_date,i.running_time,i.screening)
```
...
```python
movi = Movie.objects.order_by('-opennig_date')
for i in movi:
...
```python
movi = Movie.objects.order_by('opennig_date')[:1]
for i in movi:
    print(i.director.name,i.genre.title,i.title,i.opening_date,i.running_time,i.screening)
```
...
```python
movi = Movie.objects.filter(screening=True).order_by('opening_date')
for i in movi:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
bong_ = Movie.objects.filter(director=bong).order_by('opening_date')
for i in bong_:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
mo = Movie.objects.filter(director=bong).order_by('opening_date')[1]
print(mo.director.name, mo.genre.title, mo.title, mo.opening_date, mo.running_time, mo.screening)
```
...
```python
gt(초과) = Movie.objects.filter(running_time__gt=119).order_by('running_time')
for i in gt:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
```python
gte(이상) =Movie.objects.filter(running_time__gte=119).order_by('running_time')
for i in gte:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
gt_day = Movie.objects.filter(opening_date__gt=day).order_by('opening_date')
for i in gt_day:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
moov = Movie.objects.filter(director=bong, genre=drama).order_by('opening_date')
for i in moov:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
```python
bong = Director.objects.get(name='봉준호').id
exclude_ = Movie.objects.exclude(director=bong).order_by('opening_date')
for i in exclude_:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```