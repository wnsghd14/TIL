...
```sql
-- SELECT ArtistId,COUNT(*) AS "개수" FROM albums GROUP BY ArtistId ORDER BY "개수" DESC LIMIT 1; 
SELECT Name FROM artists WHERE ArtistId = (SELECT ArtistId FROM albums GROUP BY ArtistId ORDER BY COUNT(*) DESC LIMIT 1);
```

SELECT Name FROM artists WHERE ArtistId = (SELECT ArtistId FROM(SELECT ArtistId,COUNT(*) AS "개수" FROM albums GROUP BY ArtistId ORDER BY "개수" DESC limit 1));

SELECT Name FROM artists WHERE ArtistID =(SELECT ArtistId FROM albums GROUP BY ArtistId HAVING COUNT(ArtistID)= (SELECT MAX("개수") FROM (SELECT COUNT(*) AS "개수" FROM albums GROUP BY ArtistId)));
...
```
SELECT Name FROM genres WHERE GenreID = (SELECT GenreId FROM (SELECT GenreId, COUNT(*) AS "개수" FROM tracks GROUP BY GenreId ORDER BY "개수" ASC LIMIT 1));

SELECT Name FROM genres WHERE GenreId =(SELECT GenreId FROM tracks GROUP BY GenreId HAVING COUNT(GenreId) = (SELECT MIN("개수") FROM (SELECT COUNT(*) AS "개수" FROM tracks GROUP BY GenreId)));

SELECT Name FROM genres WHERE GenreId = (SELECT GenreId FROM(SELECT GenreId, MIN(CNT) FROM (SELECT GenreId,COUNT(*) 'CNT' FROM tracks GROUP BY GenreId)));
```
